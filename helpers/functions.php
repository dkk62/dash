...
function buildPendingReportHtml(array $reportData, string $reportDate): string {
    ...
    $host = $_SERVER['HTTP_HOST'] ?? 'dashboard.taxcheapo.com';
    $logoUrl = $h($scheme . '://' . $host . appUrl('public/img/taxcheapo-logo-white.png'));

    $html .= '<div class="header">'
           . '<div style="text-align:center;padding-bottom:10px;"><img src="' . $logoUrl . '" alt="TaxCheapo" style="height:40px;width:auto;" /></div>'
...
/**
 * Wrap email content in a branded HTML template with TaxCheapo logo header.
 */
function wrapEmailHtml(string $innerHtml): string {
    $scheme = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off') ? 'https' : 'http';
    $host = $_SERVER['HTTP_HOST'] ?? 'dashboard.taxcheapo.com';
    $logoUrl = htmlspecialchars($scheme . '://' . $host . appUrl('public/img/taxcheapo-logo-white.png'), ENT_QUOTES, 'UTF-8');

    return "<html><body style='margin:0;padding:0;font-family:Arial,sans-serif;color:#333;background-color:#f8f9fa;'>"
         . "<table width='100%' cellpadding='0' cellspacing='0' style='background-color:#f8f9fa;'><tr><td align='center'>"
         . "<table width='600' cellpadding='0' cellspacing='0' style='max-width:600px;width:100%;'>"
         . "<tr><td style='background-color:#212529;padding:20px;text-align:center;border-radius:8px 8px 0 0;'>"
         . "<img src='{$logoUrl}' alt='TaxCheapo' style='height:40px;width:auto;' />"
         . "</td></tr>"
         . "<tr><td style='background-color:#ffffff;padding:30px;'>"
         . $innerHtml
         . "</td></tr>"
         . "<tr><td style='padding:15px;text-align:center;font-size:12px;color:#999;'>&copy; TaxCheapo</td></tr>"
         . "</table></td></tr></table>"
         . "</body></html>";
}

/**
 * Convert plain text email body to HTML paragraphs (preserves line structure).
 */
function textToEmailHtml(string $text): string {
